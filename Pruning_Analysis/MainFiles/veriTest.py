...
def funcAddCexPruneCandidateSet(dfOrig, tree_model):
    ...
    with open('typePrunefile.txt') as fileCond:
        prune_file_content = fileCond.readlines()

    prune_file_content = [x.strip() for x in prune_file_content]		
    
    if(prune_file_content[0] == 'branch'):	
    ...
    elif(prune_file_content[0] == 'feature'):	
...
def funcMain(model, df, noOfFe, MAX_SAMPLES):
    ...
    with open('typeMonfile.txt') as fileCond:
        type_file_content = fileCond.readlines()

    type_file_content = [x.strip() for x in type_file_content]	
    if((type_file_content[0] == 'strong') or (type_file_content[0] == 'weak')):
        type_monotonicity = type_file_content
    else:
        raise Exception('Wrong monotonicity type given')
        sys.exit(1)	
